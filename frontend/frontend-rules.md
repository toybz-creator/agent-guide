...
- Define each feature module's public API with barrel exports from its root `index` file. Code outside the module must import only from that root API instead of deep-importing components, hooks, stores, schemas, actions, query definitions, or helpers from their definition files. Enforce this with lint or architecture rules such as `no-restricted-imports`, `import/no-internal-modules`, dependency-cruiser, Nx module boundaries, or the project's equivalent tool. Document any allowed exceptions for tests, stories, generated code, or intentionally shared internals.
