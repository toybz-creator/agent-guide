...
\`\`\`bash
npx pag task --id YYYYMMDD-short-kebab-slug --title "Task title"
\`\`\`

Validate progress with:

\`\`\`bash
npx pag verify-task --id YYYYMMDD-short-kebab-slug --stage rewrite
npx pag verify-task --id YYYYMMDD-short-kebab-slug --stage planning
npx pag verify-task --id YYYYMMDD-short-kebab-slug --stage complete
\`\`\`
...
Follow the numbered rules in Production Agent Guide. Create or resume the task implementation guide under harness/planning-notes/<task-id>/, run Rewrite, Planning, Implementation, Evaluation, Feedback, and Iteration, and keep affected living harness documents synchronized.
\`\`\`

The canonical detailed structure is generated by:

\`\`\`bash
npx pag task --id <task-id> --title "<title>"
\`\`\`
...
function printHelp() {
  console.log(`Production Agent Guide
