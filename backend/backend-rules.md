...
- Define each bounded module's public API with barrel exports from its root `index` file. Other modules must import only from that root API instead of deep-importing controllers, services, repositories, adapters, entities, DTOs, policies, jobs, or helpers from their definition files. Enforce this boundary with lint or architecture rules such as `no-restricted-imports`, `import/no-internal-modules`, dependency-cruiser, Nx module boundaries, or the project's equivalent tool. Document any allowed exceptions for tests, migrations, generated code, or intentionally shared internals.
